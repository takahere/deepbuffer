...
export const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // AuthProvider will detect change and redirect via PrivateRoute logic or we can push
      navigate('/'); 
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 px-4">
      <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-xl shadow-lg p-8 border border-slate-200 dark:border-slate-700">
        <h2 className="text-2xl font-bold text-center text-slate-800 dark:text-white mb-6">DeepBufferにログイン</h2>
